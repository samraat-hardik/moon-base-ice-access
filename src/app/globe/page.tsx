...
function GlobeContent() {
  ...
  const exitFullScreen = () => router.push("/globe");

  return (
    <>
      {isFullScreen && (
        <MoonGlobeScene defaultView="globe" psrs={psrs} fullScreen />
      )}
      <main
        className={
          isFullScreen
      ...
      >
        ...
        >
          {!isFullScreen && (
            <div className="animate-fade-in">
              <h1 className="text-xl font-semibold text-white mb-1">3D Moon Globe</h1>
              <p className="text-white/70 text-sm">
                Interactive lunar surface from NASA&apos;s Lunar Reconnaissance Orbiter (LRO). Drag to rotate, scroll to zoom.
                {" "}
                <Link href="/globe?fullscreen=1" className="text-lunar-ice hover:underline">
                  Full screen
                </Link>
                {" · "}
                <Link href="/globe/south-pole" className="text-lunar-ice hover:underline">
                  Focus South Pole (Artemis)
                </Link>
                {" · "}
          ...
          {!isFullScreen && <MoonGlobeScene defaultView="globe" psrs={psrs} />}
            ...
            <>
              <div className="flex items-center gap-4">
                <button
                  ...
                  className="px-3 py-2 rounded-lg bg-white/15 text-white text-sm font-medium hover:bg-white/25 transition-colors"
                >
                  Exit full screen
                </button>
                <Link
                  href="/globe"
                  className="px-3 py-2 rounded-lg bg-white/10 text-white/90 text-sm hover:bg-white/20"
                >
                  Back to Globe
                </Link>
              </div>
        ...
        {!isFullScreen && (
          <>
            <div className="max-w-4xl w-full mx-auto mt-2 flex items-center gap-4 text-xs text-white/60">
              <span><span style={{ color: COLOR_HIGH_ICE }}>●</span> High ice priority</span>
              <span><span style={{ color: COLOR_PSR }}>●</span> PSR</span>
            </div>
            <div className="max-w-4xl w-full mx-auto text-xs text-white/50 border-t border-white/10 pt-3 mt-2">
              Model: Moon_NASA_LRO_8k_Flat.glb (NASA SVS 14959). Credit: NASA&apos;s Goddard Space Flight Center, LRO.
            </div>
          </>
        )}
      </main>
